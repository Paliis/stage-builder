---
title: "Порушення під час інспекції COF (walkthrough)"
card_id: "C100"
slug: "walkthrough-interference"
category: penalties
locale: uk
discipline: mini_rifle
control_values: "Перевірити PDF Mini Rifle Jan 2026 — 8.7.2 та 8.7.3 (окремі заборони та наслідки)"
ipsc_edition: "Jan 2026"
ipsc_refs:
  - rule: "8.7.2"
    note: "Під час офіційної інспекції COF / walkthrough — Mini Rifle Jan 2026"
  - rule: "8.7.3"
    note: "Додаткові обмеження під час walkthrough — Mini Rifle Jan 2026"
primary_url: "https://ipsc-pl.org/images/przepisy_2026/IPSC%20Mini%20Rifle%20Competition%20Rules%20-%20Jan%202026%20Edition%20-%20Final%2029%20Dec%202025.pdf"
fpsu_refs:
  - rule: "IX §7"
    note: "Прицілювання та ознайомлення з вправою"
    url: "https://upsf.org.ua/rules/upsf/2020/09-course-of-fire#7-%D0%BF%D1%80%D0%B8%D1%86%D1%96%D0%BB%D1%8E%D0%B2%D0%B0%D0%BD%D0%BD%D1%8F-%D1%82%D0%B0-%D0%BE%D0%B7%D0%BD%D0%B0%D0%B9%D0%BE%D0%BC%D0%BB%D0%B5%D0%BD%D0%BD%D1%8F-%D1%96%D0%B7-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2%D0%BE%D1%8E"
  - rule: "V §2"
    note: "Письмові брифінги до вправ"
    url: "https://upsf.org.ua/rules/upsf/2020/05-stage-information#2-%D0%BF%D0%B8%D1%81%D1%8C%D0%BC%D0%BE%D0%B2%D1%96-%D0%B1%D1%80%D0%B8%D1%84%D1%96%D0%BD%D0%B3%D0%B8-%D0%B4%D0%BE-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2"
fpsu_delta_verified: false
reviewer: ""
review_date: ""
draft_source: llm
status: draft
---

## Що це

Під час **групової інспекції** / **walkthrough** сцени діють **особливі** обмеження: що можна робити з **mini rifle**, тілом, як тестувати позиції та лінії вогню. **8.7.2** і **8.7.3** у **Mini Rifle Jan 2026** описують різні аспекти цієї фази (не змішуйте їх з **8.7.1** — див. картку **C69**). Наслідки порушень — **лише з PDF Mini Rifle**, а не лише з **Handgun** чи повного **Rifle** **розділу 8**.

## Що робить суддя змагань (RO)

1. **Провести walkthrough** згідно з брифінгом матчу та **розділом 8.7**: чіткі інструкції групі, зона перебування RO.
2. **Зупиняти порушення** негайно (слово, жест, тестування заборонених дій) — мінімізувати «факти на сцені» до змагання.
3. **Застосувати санкцію** за відповідний підпункт (**8.7.2** / **8.7.3**) після звірки з PDF; занести в протокол / заліковий лист так, як вимагає регламент матчу.
4. **При повторних або системних порушеннях** — **CRO/RM**.
5. **Не плутати** з дисциплінарними **DQ** з **розділу 10**, якщо PDF відносить епізод лише до процедурних санкцій **8.7**.

## IPSC (Jan 2026)

- Читати **8.7** цілком у PDF **Mini Rifle**.

## Локально (ФПСУ)
**Первинник — Правила ФПСУ 2020 (офіційний зміст на upsf.org.ua):**

- [IX §7 — Прицілювання та ознайомлення з вправою](https://upsf.org.ua/rules/upsf/2020/09-course-of-fire#7-%D0%BF%D1%80%D0%B8%D1%86%D1%96%D0%BB%D1%8E%D0%B2%D0%B0%D0%BD%D0%BD%D1%8F-%D1%82%D0%B0-%D0%BE%D0%B7%D0%BD%D0%B0%D0%B9%D0%BE%D0%BC%D0%BB%D0%B5%D0%BD%D0%BD%D1%8F-%D1%96%D0%B7-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2%D0%BE%D1%8E)
- [V §2 — Письмові брифінги до вправ](https://upsf.org.ua/rules/upsf/2020/05-stage-information#2-%D0%BF%D0%B8%D1%81%D1%8C%D0%BC%D0%BE%D0%B2%D1%96-%D0%B1%D1%80%D0%B8%D1%84%D1%96%D0%BD%D0%B3%D0%B8-%D0%B4%D0%BE-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2)


Час і формат walkthrough можуть бути регламентовані додатково; дотримуйтесь **MD**/регламенту. **`fpsu_delta_verified`**: **false** до рев’ю за **RO_HELPER_V0 §4.1**.

## Типові помилки

- **Один штраф на все:** **8.7.2** vs **8.7.3** — різні ситуації в PDF.
- **Пізня реакція:** заборонене тестування під час walkthrough — потім важко відновити справедливість.
- **Плутання з C69:** чітко розділяйте фази **8.7.1** та **8.7.2/8.7.3**.

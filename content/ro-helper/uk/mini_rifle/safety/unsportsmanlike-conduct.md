---
title: "Неспортивна поведінка"
card_id: "C143"
slug: "unsportsmanlike-conduct"
category: safety
locale: uk
discipline: mini_rifle
control_values: "Перевірити PDF Mini Rifle Jan 2026 — 10.6 (види поведінки, процедура, DQ)"
ipsc_edition: "Jan 2026"
ipsc_refs:
  - rule: "10.6"
    note: "Unsportsmanlike conduct — Mini Rifle; повний текст у PDF"
primary_url: "https://ipsc-pl.org/images/przepisy_2026/IPSC%20Mini%20Rifle%20Competition%20Rules%20-%20Jan%202026%20Edition%20-%20Final%2029%20Dec%202025.pdf"
fpsu_refs:
  - rule: "XI §6"
    note: "Дискваліфікація — неспортивна поведінка"
    url: "https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#6-%D0%B4%D0%B8%D1%81%D0%BA%D0%B2%D0%B0%D0%BB%D1%96%D1%84%D1%96%D0%BA%D0%B0%D1%86%D1%96%D1%8F-%D0%B7%D0%B0-%D0%BD%D0%B5%D1%81%D0%BF%D0%BE%D1%80%D1%82%D0%B8%D0%B2%D0%BD%D1%83-%D0%BF%D0%BE%D0%B2%D0%B5%D0%B4%D1%96%D0%BD%D0%BA%D1%83"
fpsu_delta_verified: false
reviewer: ""
review_date: ""
draft_source: llm
status: draft
---

## Що це

**10.6** **IPSC Mini Rifle** поширює правила про **неспортивну поведінку** на юних спортсменів і цей дивізіон. **Батьки / тренери** також можуть спричинити залучення **MD**, якщо втручаються в роботу суддів — дотримуйтесь **10.6** і політики захисту дітей від **MD**. **C140** — handgun.

## Що робить суддя змагань (RO)

1. **Факти до RM:** уникати суперечок з неповнолітніми чи дорослими «на ходу» — документувати й ескалювати.
2. **Відокремити** емоції тренування від порушень **10.6** — усе одно потрібна відповідність **PDF**.
3. **Звітність у федерацію**, якщо потрібно після **DQ** або серйозного інциденту.

## IPSC (Jan 2026)

- **10.6 Mini Rifle** — редакційно звірити з PDF **Rifle**.

## Локально (ФПСУ)
**Первинник — Правила ФПСУ 2020 (офіційний зміст на upsf.org.ua):**

- [XI §6 — Дискваліфікація — неспортивна поведінка](https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#6-%D0%B4%D0%B8%D1%81%D0%BA%D0%B2%D0%B0%D0%BB%D1%96%D1%84%D1%96%D0%BA%D0%B0%D1%86%D1%96%D1%8F-%D0%B7%D0%B0-%D0%BD%D0%B5%D1%81%D0%BF%D0%BE%D1%80%D1%82%D0%B8%D0%B2%D0%BD%D1%83-%D0%BF%D0%BE%D0%B2%D0%B5%D0%B4%D1%96%D0%BD%D0%BA%D1%83)


**`fpsu_delta_verified`**: **false** до **RO_HELPER_V0 §4.1**.

## Типові помилки

- **Зменшення** значення поведінки як «просто діти» — **10.6** стосується перелічених дій незалежно від віку.

---
title: "Procedural penalty cap"
card_id: "C80"
slug: "procedural-cap"
category: penalties
locale: en
discipline: mini_rifle
control_values: "Verify Mini Rifle Jan 2026 PDF — 10.2.3 (what the cap attaches to)"
ipsc_edition: "Jan 2026"
ipsc_refs:
  - rule: "10.2.3"
    note: "Maximum procedural in a defined context — Mini Rifle Jan 2026; PDF only"
primary_url: "https://ipsc-pl.org/images/przepisy_2026/IPSC%20Mini%20Rifle%20Competition%20Rules%20-%20Jan%202026%20Edition%20-%20Final%2029%20Dec%202025.pdf"
fpsu_refs:
  - rule: "XI §1"
    note: "Procedural penalties — general"
    url: "https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#1-%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D0%B4%D1%83%D1%80%D0%BD%D1%96-%D1%88%D1%82%D1%80%D0%B0%D1%84%D0%B8-%D0%B7%D0%B0%D0%B3%D0%B0%D0%BB%D1%8C%D0%BD%D1%96-%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0"
fpsu_delta_verified: true
reviewer: "Gemini-Rules-2026"
review_date: "2026-04-20"
draft_source: llm
status: reviewed
---

## What this is

**10.2.3** limits **how many** times (or to what ceiling) **procedural** penalties may accrue in a defined context — e.g. per **shot**, **target**, **stage**, or another unit **exactly as the Jan 2026 Mini Rifle PDF states**. This is a **cap rule** for a specific class of violations — read **Mini Rifle** **Chapter 10**, not **Handgun** or full **Rifle** alone.

## What the Range Officer (RO) does

1. **Count procedurals** from **COF** facts under the applicable **10.2** / **10.1** clauses.
2. **Apply the cap:** if the total exceeds the **10.2.3** limit, **trim** scoring as the PDF requires (do not arbitrarily reduce without a rule basis).
3. **Scoresheet** clarity for scoring: how many episodes occurred vs how many **count** after the cap.
4. **If unsure** whether the same “string” of violations sits under one cap — **CRO/RM**.
5. **Avoid mixing** with multiple different clauses on one episode (depends on the PDF).

## IPSC (Jan 2026)


### 10.2.3 — cap: per occurrence vs per shot

- **10.2.3** caps procedural penalties within the specific “container” defined by the rule (per **occurrence**, per **shot**, per **target**, etc.).
- Typical case: **both feet** out at the same moment is a **single occurrence** → maximum **one** procedural, unless the rule explicitly states otherwise.
- If the violation created a **significant advantage**, a different rule path may apply (see `significant-advantage`).
- Keep the distinction clear: **one penalty per occurrence** vs **one penalty per shot** are different counting modes.

- Anchor: **10.2.3** in **Mini Rifle**; read together with the sub-rules the cap references.
- Match **chief scoring** / **RM** may issue uniform interpretation — follow match briefings.

## Local (FPSU)
**Primary source — FPSU Rules 2020 (official text on upsf.org.ua):**

- [XI §1 — Procedural penalties — general](https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#1-%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D0%B4%D1%83%D1%80%D0%BD%D1%96-%D1%88%D1%82%D1%80%D0%B0%D1%84%D0%B8-%D0%B7%D0%B0%D0%B3%D0%B0%D0%BB%D1%8C%D0%BD%D1%96-%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0)


If FPSU mirrors IPSC with deltas, the cap may differ; verify FPSU text. **`fpsu_delta_verified`**: **true** until **RO_HELPER_V0 §4.1** review.

## Common mistakes

- **Ignoring the cap:** extra procedurals on the scoresheet vs the PDF.
- **Wrong “container”:** cap per shot vs per target — category error.
- **Vague logging:** no episode breakdown — cannot audit **10.2.3**.

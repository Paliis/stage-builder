---
title: "Mandatory reload violation"
card_id: "C54"
slug: "mandatory-reload"
category: penalties
locale: en
discipline: mini_rifle
control_values: "Verify Mini Rifle Jan 2026 PDF — procedural count per episode (10.2.4)"
ipsc_edition: "Jan 2026"
ipsc_refs:
  - rule: "10.2.4"
    note: "WSB mandatory reload not performed as required — Mini Rifle Jan 2026"
primary_url: "https://ipsc-pl.org/images/przepisy_2026/IPSC%20Mini%20Rifle%20Competition%20Rules%20-%20Jan%202026%20Edition%20-%20Final%2029%20Dec%202025.pdf"
fpsu_refs:
  - rule: "XI §2"
    note: "Procedural penalty examples"
    url: "https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#2-%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D0%B4%D1%83%D1%80%D0%BD%D1%96-%D1%88%D1%82%D1%80%D0%B0%D1%84%D0%B8-%D0%BE%D0%BA%D1%80%D0%B5%D0%BC%D1%96-%D0%BF%D1%80%D0%B8%D0%BA%D0%BB%D0%B0%D0%B4%D0%B8"
  - rule: "IX §4"
    note: "Loading / reloading"
    url: "https://upsf.org.ua/rules/upsf/2020/09-course-of-fire#4-%D0%B7%D0%B0%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F-%D0%BF%D0%B5%D1%80%D0%B5%D0%B7%D0%B0%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F-%D0%B0%D0%B1%D0%BE-%D1%80%D0%BE%D0%B7%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F"
fpsu_delta_verified: true
reviewer: "Gemini-Rules-2026"
review_date: "2026-04-20"
draft_source: llm
status: reviewed
---

## What this is

A **mandatory reload** in the **WSB** means the competitor **must** perform a reload (type, point in the **COF**, count, etc.) exactly as written. If the competitor **fails** that requirement, the penalty anchor is **10.2.4** in **IPSC Mini Rifle** — exact conditions and **procedural** count **only** from the **Jan 2026** PDF. Use **Mini Rifle** **Chapter 10**, not **Handgun** or full **Rifle** reload examples, when checking the text.

## What the Range Officer (RO) does

1. **Read the WSB:** where reload is **mandatory**; single episode or several; exceptions (e.g. after malfunction) if stated.
2. **Record facts:** whether a reload occurred; whether it matches the WSB (tactical vs administrative — **per WSB and rules text only**).
3. **Apply the penalty:** scoresheet per **10.2.4** after PDF check — do not mis-bucket under **10.2.2** if the issue is reload, not position.
4. **Brief the competitor:** which WSB requirement was missed (**9.6** for challenges).
5. **Document:** WSB clause + moment in **COF**.

## IPSC (Jan 2026)


### 10.2.4 — penalty until reload (per shot)

- If the WSB requires a reload and the competitor **keeps firing without reloading**, apply the penalty **per shot** until the reload is **performed**.
- Key moment: **shots fired after the point where the WSB requires the reload**.

- Primary anchor: **10.2.4** — **IPSC Mini Rifle Competition Rules** PDF, **Chapter 10**.
- If another clause fits better — use **that** clause after reading the rulebook.

## Local (FPSU)
**Primary source — FPSU Rules 2020 (official text on upsf.org.ua):**

- [XI §2 — Procedural penalty examples](https://upsf.org.ua/rules/upsf/2020/11-penalties-and-disqualifications#2-%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D0%B4%D1%83%D1%80%D0%BD%D1%96-%D1%88%D1%82%D1%80%D0%B0%D1%84%D0%B8-%D0%BE%D0%BA%D1%80%D0%B5%D0%BC%D1%96-%D0%BF%D1%80%D0%B8%D0%BA%D0%BB%D0%B0%D0%B4%D0%B8)
- [IX §4 — Loading / reloading](https://upsf.org.ua/rules/upsf/2020/09-course-of-fire#4-%D0%B7%D0%B0%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F-%D0%BF%D0%B5%D1%80%D0%B5%D0%B7%D0%B0%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F-%D0%B0%D0%B1%D0%BE-%D1%80%D0%BE%D0%B7%D1%80%D1%8F%D0%B4%D0%B6%D0%B0%D0%BD%D0%BD%D1%8F)


WSB wording may differ; **`fpsu_delta_verified`**: **true** until **RO_HELPER_V0 §4.1**.

## Common mistakes

- **Applying** when the WSB does **not** mandate a reload at that point.
- **Vague sheet:** “no reload” without the WSB line.
- **Wrong bucket:** do not force **10.2.4** when the PDF supports a different rule.

---
title: "Course design principles"
card_id: "C309"
slug: "course-design-safety-balance"
category: match-admin
locale: en
discipline: shotgun
control_values: "Verify Shotgun Jan 2026 PDF — 1.1 (safety, balance, fair COF)"
ipsc_edition: "Jan 2026"
ipsc_refs:
  - rule: "1.1"
    note: "Course design principles — Shotgun Ch.1"
primary_url: "https://ipsc-pl.org/images/przepisy_2026/IPSC%20Shotgun%20Competition%20Rules%20-%20Jan%202026%20Edition%20-%20Final%2029%20Dec%202025.pdf"
fpsu_refs:
  - rule: "III §1"
    note: "Stage design principles"
    url: "https://upsf.org.ua/rules/upsf/2020/03-stage-design#1-%D0%BE%D1%81%D0%BD%D0%BE%D0%B2%D0%BD%D1%96-%D0%BF%D1%80%D0%B8%D0%BD%D1%86%D0%B8%D0%BF%D0%B8"
  - rule: "IV §2"
    note: "Stage design criteria"
    url: "https://upsf.org.ua/rules/upsf/2020/04-stage-safety-and-zones#2-%D0%BA%D1%80%D0%B8%D1%82%D0%B5%D1%80%D1%96%D1%97-%D0%BF%D0%BE%D0%B1%D1%83%D0%B4%D0%BE%D0%B2%D0%B8-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2"
fpsu_delta_verified: false
reviewer: ""
review_date: ""
draft_source: llm
status: draft
---

## What this is

**1.1** states **principles** for IPSC courses: **safety**, **fairness** across divisions, **balance** of shooting vs movement, etc. **ROs** rarely **design** stages but **must** stop obvious hazards or impossible **WSB** execution and report to **CRO/RM** **before** squads shoot. **C305** = **Handgun** — use the **Shotgun** PDF for **1.1**.

## What the Range Officer (RO) does

1. **During stage inspection**, check firing zones, minimum distances, prop stability — within **MD** competence.
2. **Do not start** a squad if there is **acute** danger (broken brace, target shifted into RO arc).
3. **Document** photo/description for **RM** if the stage conflicts with **1.1** or the **WSB**.
4. **Feed back** to **MD** (via **CRO**) on unreadable WSB text or impossible mandatory elements.
5. **During the match**, do not “fix” design alone — only per **RM** command.

## IPSC (Jan 2026)

- **1.1 Shotgun** as framing; concrete steel / clay rules — **10.4.7** and other **Chapter 10** rules in the **Shotgun** PDF.

## Local (FPSU)
**Primary source — FPSU Rules 2020 (official text on upsf.org.ua):**

- [III §1 — Stage design principles](https://upsf.org.ua/rules/upsf/2020/03-stage-design#1-%D0%BE%D1%81%D0%BD%D0%BE%D0%B2%D0%BD%D1%96-%D0%BF%D1%80%D0%B8%D0%BD%D1%86%D0%B8%D0%BF%D0%B8)
- [IV §2 — Stage design criteria](https://upsf.org.ua/rules/upsf/2020/04-stage-safety-and-zones#2-%D0%BA%D1%80%D0%B8%D1%82%D0%B5%D1%80%D1%96%D1%97-%D0%BF%D0%BE%D0%B1%D1%83%D0%B4%D0%BE%D0%B2%D0%B8-%D0%B2%D0%BF%D1%80%D0%B0%D0%B2)


FPSU may add local minimum distances; they must appear in **WSB/MD**. **`fpsu_delta_verified`**: **false** until **RO_HELPER_V0 §4.1** review.

## Common mistakes

- **“We’ll sort it live”** instead of stopping an unsafe stage.
- **RO moves** a target without **RM**.
- **Ignoring** multiple RO reports about the same hazard.
- Using **Handgun 1.1** **without** opening the **Shotgun** PDF.

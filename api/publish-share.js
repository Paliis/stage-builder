...
var publishShareApiHandler_exports = {};
__export(publishShareApiHandler_exports, {
  default: () => handler
});
...
function clonePenaltyZoneSet(pz) {
  return {
...
function buildStageProjectFile(snapshot) {
  return {
...
function serializeStageProject(file) {
...
var PUBLISHES_PER_DAY_PER_IP = 50;
var rateBuckets = /* @__PURE__ */ new Map();
function checkPublishRateLimit(ip) {
...
function newShareId() {
  const b = (0, import_node_crypto.randomBytes)(18);
...
function normalizePublishBody(body) {
  ...
  if (rawIdem !== void 0 && rawIdem !== null) {
...
async function handler(req, res) {
  ...
  if (typeof idemHeader === "string" && idemHeader.trim() && typeof body === "object" && body !== null) {
    const o = body;
    if (o.idempotencyKey === void 0) o.idempotencyKey = idemHeader.trim();
  }
  const normalized = normalizePublishBody(body);
  if (!normalized.ok) {
    return res.status(normalized.status).json({ error: normalized.error });
  }
  const ip = (typeof req.headers["x-forwarded-for"] === "string" ? req.headers["x-forwarded-for"].split(",")[0]?.trim() : null) || req.socket?.remoteAddress || "unknown";

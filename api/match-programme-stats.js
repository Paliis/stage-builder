...
// src/server/matchProgrammeStatsApiHandler.ts
var matchProgrammeStatsApiHandler_exports = {};
__export(matchProgrammeStatsApiHandler_exports, {
  default: () => handler
});
module.exports = __toCommonJS(matchProgrammeStatsApiHandler_exports);
...
// src/domain/computeMinRounds.ts
function computeMinRounds(targets) {
  let sum = 0;
...
// src/domain/matchProgrammeStats.ts
...
function computeMatchStageTargetCounts(targets) {
  const out = {
    paper: 0,
    metalPlates: 0,
    ceramic: 0,
...
function ammoLabelFromBriefing(allowedAmmo) {
  const t = allowedAmmo.trim();
  if (!t) return "\u2014";
  const head2 = t.split(/[(\n]/)[0]?.trim() ?? "";
  if (!head2) return "\u2014";
  const lower = head2.toLowerCase();
  ...
  return head2.length > 28 ? `${head2.slice(0, 27)}\u2026` : head2;
}
function pointsFromBriefingAndTargets(briefing, targets) {
  ...
  return 5 * (counts.poppers + counts.miniPoppers + counts.metalPlates + counts.ceramic) + 10 * counts.paper;
}
function shotsFromBriefingAndTargets(briefing, targets) {
  const fromBrief = parseBriefingOptionalPositiveInt(briefing.recommendedShots);
  if (fromBrief !== null) return fromBrief;
  return computeMinRounds(targets);
}
function matchStageStatRowFromProject(sortOrder, project) {
...
function aggregateMatchProgrammeStats(partialRows) {
  const totalPoints = partialRows.reduce((s, r) => s + r.points, 0);
  const rows = partialRows.map((r) => ({
    ...r,
    matchPercent: totalPoints > 0 ? Math.round(r.points / totalPoints * 1e3) / 10 : 0
  }));
  const totals = {
    paper: 0,
    metalPlates: 0,
...
// src/server/loadMatchProgrammeStats.ts
var MATCH_ID_UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
async function loadMatchProgrammeStats(supabase, matchId) {
  ...
  const shareIds = stages.map((s) => typeof s.share_stage_id === "string" ? s.share_stage_id.trim() : "").filter(Boolean);
  ...
  }).filter((r) => r !== null).sort((a, b) => a.sortOrder - b.sortOrder);

...
**Публікація (POST):** логіка в **`src/server/publishShareApiHandler.ts`**; у деплой вона збирається в **`api/publish-share.js`** (esbuild у **`npm run build`**) — один CommonJS-файл для Vercel Serverless, бо «сирий» **`api/*.ts`** з імпортами з **`src/`** на платформі давав **`FUNCTION_INVOCATION_FAILED`** (помилка ще до входу в handler). Змінні: **`SUPABASE_SERVICE_ROLE_KEY`**, **`SUPABASE_URL`** або **`VITE_SUPABASE_URL`**; опційно **`VITE_SHARE_PUBLIC_ORIGIN`**. Валідація/нормалізація: **`src/server/sharePublish.ts`**. Локально: спочатку **`npm run build`** (щоб з’явився **`api/publish-share.js`**), далі **`vercel dev`**, або preview/production deploy. Якщо публікація без тексту помилки від Supabase — перевірте змінні на **production** і міграцію **`shared_stages`**. Якщо **не JSON** у відповіді — окрім SW (**`NetworkOnly`** для **`/api/*`**), перевірте логи функції на Vercel (**Invocation failed**).

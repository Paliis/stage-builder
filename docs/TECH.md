...
- **`exportBriefingPdf.ts`** + **`pdfBriefingLogos.ts`** — заголовок PDF: опційні лого (растр із `public/briefing-logos/*.svg`), **назва матчу**, заголовок документа; таблиця з компактним першим рядком (**тип вправи · рекомендовані постріли**); знімок сцени, QR, бренд під знімком (див. також попередні правки inset / відступів). **`pdfFonts.ts`** — Roboto для кирилиці.

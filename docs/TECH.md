...
- **Сітка та snap** — `GRID_SNAP_M = 0.5`; під сіткою на 2D — легка заливка **`plan2DFieldBaseRgba(fieldGroundCover3d)`** (той самий колір, що підлога в 3D, `groundCoverColorHex`); поверх — **шахматка** кроком `GRID_CHESS_M = 1` (дві тонкі заливки, лише видимий viewport; при дуже сильному віддаленні або понад ~8000 клітин у кадрі шахматку пропускаємо — лишаються лінії 0,5 м). Для центрів після перетягування: `TARGET_PLACEMENT_SNAP_M` і `PROP_PLACEMENT_SNAP_M` (0,05 м), щоб дрібно позиціонувати мішені й стикувати реквізит.

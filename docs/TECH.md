...
- **Картка 2D/3D** (`App.css` `.app__main--stage-card`): ширина з `aspect-ratio` не звужується нижче **20rem** (узгоджено з мінімальною шириною колонки сцени в сітці), щоб при «високих» полях (мала ширина / велика довжина) перегляд не ставав смужкою вужчою за контент. Поля розміру поля в шапці — текстовий ввід з фіксацією при **blur** (чернетка в стейті), щоб можна було набрати значення з клавіатури без миттєвого clamp по мінімуму.

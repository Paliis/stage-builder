...
**Публікація (POST):** **`api/publish-share.ts`** (Vercel Serverless) — **`SUPABASE_SERVICE_ROLE_KEY`**, **`SUPABASE_URL`** або **`VITE_SUPABASE_URL`**; опційно **`VITE_SHARE_PUBLIC_ORIGIN`** для абсолютних **`url`** у відповіді. Логіка валідації/нормалізації: **`src/server/sharePublish.ts`**. Локально повний стек: **`vercel dev`** (або лише деплой на Preview). Якщо в UI з’являється помилка публікації без тексту від Supabase — перевірте, що **production** на Vercel має ті самі змінні, що й preview; переконайтеся, що міграція **`shared_stages`** застосована в проєкті Supabase. Якщо повідомлення про **HTML замість JSON** — часто винен **service worker** (PWA): згенерований **`sw.js`** має окремі правила **`NetworkOnly`** для **POST** і **GET** на **`/api/*`** (інакше **POST** `/api/publish-share` міг оброблятися як навігація й віддавати **`index.html`**). Після деплою варто оновити SW (або перевірити в режимі інкогніто).

...
- **`exportBriefingPdf.ts`** — таблиця полів брифінгу, вбудований знімок 3D (після `addImage` — обведення `roundedRect` навколо кадру), QR у куті знімка, бренд-текст і URL під знімком (без накладання на PNG).

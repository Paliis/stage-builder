...
| **Application** | `src/application/` | Zustand: сцена (`stageStore`, у т. ч. **`activations`** BL-004, undo/redo через `zundo`), брифінг (`briefingStore`), чернетка `localStorage` (`sessionDraft.ts`), компонент **`SessionDraftPersist.tsx`** (підписка + debounce), хмарна бібліотека вправ (`userStagesLibrary.ts` → таблиця `user_stages`) |
...
| **Корінь UI** | `src/App.tsx` | Композиція layout, панель брифінгу з кнопкою PDF, гарячі клавіші, lazy-3D, стрічка staging, посилання на канвас через `ref` (`StageCanvasHandle`); шапка й футер — від `PortalShell`, окрім `standalone` (share) |
...
**Staging (стан 2026-06):** окремий Vercel-проєкт, той самий репозиторій `Paliis/stage-builder`, **той самий Supabase**, що й prod. У Production env staging-проєкту: `VITE_SITE_ENV=staging`, `VITE_ENABLE_MATCH_PORTAL=1`, `VITE_SHARE_PUBLIC_ORIGIN=https://stage-builder-staging.vercel.app`, `VITE_SUPABASE_*`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (як на prod). Демо-події — seed: `npm run supabase:seed:match-all`. Каталог: `/{uk|en}/matches`. **Онлайн-оплата Mono:** `api/create-payment.js`, `api/payments/webhook/mono.js` (`bodyParser: false`), `api/payments/reconcile.js`, `api/organizer-mono-payment.js` — джерела в `src/server/*ApiHandler.ts`, збірка `npm run build:api`; план — [MATCH_PAYMENTS_PLAN.md](./MATCH_PAYMENTS_PLAN.md), handoff — [CHAT_HANDOFF.md](./CHAT_HANDOFF.md).

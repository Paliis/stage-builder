...
- **3D і знімок PDF** — `a4PrintLayout.ts`: `stageViewportAspectRatio`, `pdfSnapshotPixelSize` (ширина колонки PDF × масштаб, висота з aspect). У **`StageView3D`** режим камери `pdf` дає вікно перегляду з тим самим **aspect**, що й PNG для брифінгу (плюс рамка в `App.css`); `capturePngDataUrl` рендерить у розмірі з `pdfSnapshotPixelSize` незалежно від режиму на екрані.

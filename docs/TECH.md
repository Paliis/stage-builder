...
- **`exportBriefingPdf.ts`** — таблиця полів брифінгу, вбудований знімок (після `addImage` — обведення `roundedRect` навколо кадру; джерело — 3D або 2D канвас), QR у верхньому правому куті **сторінки**, бренд-текст і URL по центру під знімком (перенос у вужчій ширині за рахунок горизонтальних inset); зазор перед таблицею та padding комірок налаштовуються константами у файлі; вертикальний старт знімка не вище за низ QR.

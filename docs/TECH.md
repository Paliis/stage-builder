...
- **Закріплені розміри** (`planDimensions`) — створення режимом «Розміри»; переміщення endpoints або сегменту (рух усій лінії), виділення кліком, **Delete/Backspace** прибирають лінію; мутація `setPlanDimensionLineEnds` у `stageStore` (Temporal undo); маркери кінців на канвасі компактні (`dimensionPlanEndpointDotPx`).
- **Штрафна лінія та обертання** — екранний радіус помаранчевої точки розтягування кінця лінії та синьої ручки обертання (↻) збігається з радіусом **невиділеної** вершини контуру штрафної зони (`penaltyContourVertexHandleRadiusPx` у `StageCanvas.tsx`), щоб ручки менше перекривали сусідні об’єкти; зона влучення кліком лишається через `pickHandle` / `TOUCH_PICK_MIN_PX`.

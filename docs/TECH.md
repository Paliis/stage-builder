...
- **`exportBriefingPdf.ts`** + **`pdfBriefingLogos.ts`** — заголовок PDF: лого зліва (`public/briefing-logos/fpsu.png`, `ipsc.png`; спроба потім `.svg`); для широкого аркуша ФПСУ **UA+LAT** у PDF використовується **ліва половина** (україномовна емблема); між лого й зоною QR — по центру смуги **назва матчу**, нижче **заголовок документа**; QR справа вгорі; таблиця з компактним першим рядком; знімок, бренд під знімком. **`pdfFonts.ts`** — Roboto.

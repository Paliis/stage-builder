...
- **`exportBriefingPdf.ts`** + **`pdfBriefingLogos.ts`** — заголовок PDF: лого зліва; між лого й зоною QR — по центру смуги **назва матчу** (якщо є), нижче **заголовок документа** (назва вправи); QR справа вгорі; таблиця з компактним першим рядком; знімок, бренд під знімком. **`pdfFonts.ts`** — Roboto.

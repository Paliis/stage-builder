...
- **Закріплені розміри** (`planDimensions`) — створення режимом «Розміри»; переміщення endpoints або сегменту (рух усій лінії), виділення кліком, **Delete/Backspace** прибирають лінію; мутація `setPlanDimensionLineEnds` у `stageStore` (Temporal undo); маркери кінців — `planContactHandleRadiusPx` (збільшені для активно виділеної лінії).
- Загально **«точки контакту» 2D** (обертання, штрафна лінія, вершини штрафної зони, чернетки, підказка вузла сітки, активне джерело активацій, маркери виміру тощо) малюються узгоджено через **`planContactHandleRadiusPx`** у **`StageCanvas.tsx`**, окремо зберігають зону влучення кліком **pick**/ **TOUCH_PICK_MIN_PX**.

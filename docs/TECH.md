...
**Публікація (POST):** логіка в **`src/server/publishShareApiHandler.ts`**; артефакт **`api/publish-share.js`** збирається esbuild у **`npm run build`** і **закомічений у репозиторій**, щоб Vercel гарантовано підхопив serverless-файл (ігнор у `.gitignore` давав відсутній маршрут **`/api/publish-share`** → **404** і порожня **`shared_stages`**). Змінні: **`SUPABASE_SERVICE_ROLE_KEY`**, **`SUPABASE_URL`** або **`VITE_SUPABASE_URL`**; опційно **`VITE_SHARE_PUBLIC_ORIGIN`**. Валідація: **`src/server/sharePublish.ts`**. Після змін у handler перезапустіть **`npm run build`** і закомітьте оновлений **`api/publish-share.js`**. Якщо публікація без тексту від Supabase — змінні на **production** і міграція **`shared_stages`**. Якщо **не JSON** — SW (**`NetworkOnly`** для **`/api/*`**) або логи функції на Vercel.

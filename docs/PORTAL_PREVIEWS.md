...
- **Генерація файлу:** режим **`contain`** у `portal-preview-prepare.mjs` (цілий скрін у кадрі
  `1280×800`, підложка `#f1f5f9`), щоб не відрізати краї UI. У `PortalHome.css` для прев’ю
  використовується **`object-fit: contain`**, щоб не було подвійного кропу в картці.

...
| **MA-B02** | Потік pending → confirmed узгоджено з політикою публічного ростера | candidate | BL-026 | Див. `participant_list_visibility` |
...
| ID | Задача | Статус | BL | Примітки |
|----|--------|--------|-----|----------|
| **MA-C01** | Прив’язка вправи до матчу (share `/v/:id`, метадані) | candidate | BL-027 | `match_stage_links`, групи share |
| **MA-C02** | Оновлення прив’язки до останнього view у групі (RPC / кнопка в UI) | candidate | BL-027 | `organizer_refresh_match_stage_link_latest` |
...
| **MA-D02** | Регресія / фікстури порівняння з еталонним `.psc` | candidate | BL-028 | Vitest + `scripts/practiscore/` |
...
| Дата | Зміни |
|------|--------|
| 2026-05-05 | Ростер організатора: один список статусу **очікує / підтверджено** (таблиця та дошка); окрема оплата/примітка прибрані з UI; **`payment_note`** та **`payment_received`** залишаються в БД (імпорт PSC тощо). |

...
- **`match_squads` / `match_stage_links`:** читання, якщо матч видимий (**`published`** для `anon` — тому публічна картка матчу в порталі може показати **програму** як упорядкований список з **`match_stage_links`**, із посиланнями на режим перегляду `/v/:share_stage_id`); **зміни** — лише активний організатор того матчу (після платформеної міграції політики використовують **`match_organizer_write_allowed`**).

...
| **`participant_registration_defaults`** | Один рядок на **стрільця**: типові поля (`division`, `classification_grade`, **`power_factor`**, **`region`**, **`categories`** `text[]`, **`weapon_class`**) для автопідстановки та експорту; якщо заявки на цей матч ще немає — префіл у публічну форму (**`/{locale}/account`** або картка матчу). |

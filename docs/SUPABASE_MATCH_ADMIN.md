...
7. **`supabase/migrations/20260506141000_organizer_registration_roster_rpc.sql`** (+ оновлення **`20260507120000_fetch_organizer_roster_created_at.sql`**, **`20260512130000_fetch_organizer_roster_payment_note.sql`**, **`20260518110000_match_registration_payment_received.sql`**, **`20260522100000_registration_phone_weapon_region.sql`**, **`20260526100000_organizer_roster_participant_defaults_name.sql`**, **`20260527100000_drop_registration_weapon_details.sql`**) — RPC **`fetch_organizer_match_registration_roster(p_match_id)`** для сторінки організатора: **`display_name`** спершу з **`participant_registration_defaults`** (**`last_name`** + **`first_name`**), інакше **`match_admin_profiles.display_name`**; далі **`registration_created_at`**, **`payment_note`**, **`payment_received`**, **`phone`** / регіон з **`match_registrations`**.
...
16. **`supabase/migrations/20260522100000_registration_phone_weapon_region.sql`** — **`participant_registration_defaults`**: **`phone`**; **`match_registrations`**: **`phone`**, знімок **`competitor_region`** при поданні заявки; RPC **`fetch_organizer_match_registration_roster`** отримував ці поля для UI організатора (поле **`weapon_details`** з цього набору прибрано в **`20260527100000_drop_registration_weapon_details.sql`**).
17. **`supabase/migrations/20260524120000_match_registrations_competitor_update_cancelled.sql`** — RLS **`UPDATE`** для стрільця: **`status IN ('pending', 'cancelled')`** (альтернатива до delete+insert для старих **`cancelled`** рядків).
18. **`supabase/migrations/20260525100000_match_registrations_competitor_delete.sql`** — **`GRANT DELETE … TO authenticated`** і політика **`match_registrations_delete_competitor_own`**: стрілець видаляє **власний** рядок у статусі **`pending`** або **`cancelled`**, якщо **`confirmed_*` IS NULL** — **відізвання з картки = `DELETE`**; повторна реєстрація = **`INSERT`** (нема конфлікту з **`UNIQUE`**).
19. **`supabase/migrations/20260526100000_organizer_roster_participant_defaults_name.sql`** — у **`fetch_organizer_match_registration_roster`** поле **`display_name`** для UI «Заявки» будується з прізвища та імені стрільця (**`participant_registration_defaults`**), за відсутності — з **`match_admin_profiles.display_name`**.
20. **`supabase/migrations/20260527100000_drop_registration_weapon_details.sql`** — видалено **`weapon_details`** з **`match_registrations`** та **`participant_registration_defaults`**; RPC **`fetch_organizer_match_registration_roster`** більше не повертає цю колонку.
...
| **`match_registrations`** | Одна заявка на пару (**match**, **стрілок**): `division`, клас, PF, `categories` JSONB, `status` pending/confirmed/cancelled (організатор може ставити **`cancelled`**), `payment_note`, **`phone`**, **`competitor_region`**, `confirmed_at/by`. **Відіза стрільця** з картки матчу: **`DELETE`** рядка **`pending`** або **`cancelled`** (до підтвердження), потім знов **`INSERT`** — див. **`20260525100000_...`**. Старий **`cancelled`** рядок перед повторною підачею клієнт також **`DELETE`**-ить, потім **`INSERT`**. |
| **`participant_registration_defaults`** | Один рядок на **стрільця**: типові поля (`division`, `classification_grade`, **`power_factor`**, **`region`**, **`categories`** `text[]`, **`weapon_class`**, **`phone`**, **`first_name`**, **`last_name`**, **`avatar_url`**) для автопідстановки, фото та експорту (ім’я/прізвище → PractiScore); якщо заявки на цей матч ще немає — префіл у публічну форму (**`/{locale}/account`** або картка матчу). |
...
- **`match_registrations`:** стрілець читає/створює свою заявку на **published** матч; **відмова від участі** — RPC **`withdraw_my_match_registration`** (**`20260531120000_...`**): **`pending`** → **`DELETE`**, **`confirmed`** → **`cancelled`** (очищення **`confirmed_*`**); повторна реєстрація після **`cancelled`** — **`DELETE`** старого рядка + **`INSERT`**. Для **`pending`/`cancelled`** без підтвердження також **`DELETE`** політика **`20260525100000_...`**. Організатор читає та **оновлює** лише коли активний; організатор — підтвердження офлайн оплати (`payment_note`, `status`, `confirmed_*`). **Anonymous** таблицю не бачить (списки без логіну — окремий RPC пізніше).

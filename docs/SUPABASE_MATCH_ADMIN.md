...
**Матч «Тестовий матч (CLI)» до 65 стрільців:** **`supabase/seed/match_admin_fill_test_match_cli_65_shooters.sql`** створює синтетичних учасників (`confirmed`) і розподіляє їх по скводах за залишком місткості (**сума `capacity` ≥ 65**). За замовчуванням шукає матч за **`title`**, або задай **`p_match_id`** у блоці `DECLARE`. CLI (**після link**): **`npm run supabase:seed:test-match-cli-65`**. Для рядків сидера виставлено **`payment_received = true`** (публічна колонка «Підтвердження» на картці матчу); повторний запуск оновлює вже наявні рядки з тим самим `payment_note`. Email формату `stagebuilder.cli65.<random>@local.test`, пароль **`SeedOnly_ChangeMe_9`** — змінити або видалити після тесту.
...
Публічна сторінка матчу показує таблицю **Учасники** лише якщо в картці матчу **`participant_list_visibility = open`** і заявки **confirmed** (`fetch_public_match_roster`, міграція **`20260520120000_public_roster_categories_payment.sql`**: сквод, ім’я, дивізіон, **категорії** (`categories` JSONB), **payment_received** без `payment_note`). Після локального редагу матчу в UI перевір видимість, якщо список зник. **Підтвердити** очікуючі заявки можна в організаторському розділі «Заявки» (`/matches/my/:id/roster`) — лише підтверджені потрапляють у цю таблицю.

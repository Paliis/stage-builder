...
- У **`PortalShell`**: «Увійти» або в один ряд — бейджі **Учасник** / **Організатор** (за тими ж правилами, що нижче), **іконка облікового запису** (email лише в підказці та `aria-label`), кнопка виходу. Статус **`pending`** у шапку **не виводиться**. Компактний режим (**гамбургер**, виїзна панель) якщо **вузьке вікно** (`matchMedia` **≤ 959px**) **або** вузький фактичний ряд хедера (**`ResizeObserver`**, те саме — наприклад у вбудованому прев’ю Cursor із широким зовнішнім вікном та вузьким iframe). На ширших рядках — блок праворуч **flex**, без **`display: contents`** (стабільніше в браузерах).

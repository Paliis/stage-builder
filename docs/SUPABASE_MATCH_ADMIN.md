...
- У **`PortalShell`**: «Увійти» або в один ряд — бейджі **Учасник** / **Організатор** (за тими ж правилами, що нижче), **іконка облікового запису** (email лише в підказці та `aria-label`), кнопка виходу. Статус **`pending`** у шапку **не виводиться**. На вузьких екранах (типово **&lt; 960px**) первинна навігація, обліковий запис і перемикач мови їдуть у **виїзну панель під гамбургер**; під десктопом група навігації розгортається в одну смугу через **`display: contents`**.

...
11. **`supabase/migrations/20260510120000_participant_registration_defaults.sql`** — таблиця **`participant_registration_defaults`** (типові **`division`** / **`classification_grade`** / **`power_factor`** для префілу форми реєстрації на картці опублікованого матчу; **`SELECT` / `INSERT` / `UPDATE` / `DELETE`** лише власного рядка **`user_id`**).
...
| **`participant_registration_defaults`** | Один рядок на **стрільця**: типові текстові поля заявки (`division`, `classification_grade`) та **`power_factor`** (NULL / MAJOR / MINOR) для автопідстановки у публічну форму, якщо заявки на цей матч ще немає (редагується в **`/{locale}/account`** або вручну на картці). |
...
**Вправи матчу (програма):** на сторінці редагування організатора **`/{locale}/matches/my/:matchId`** блок **«Вправи програми»** — вставка URL перегляду **`/v/:id`** або короткого id (`s…`), порядок (↑/↓), прибирання рядка; **«Оновити до останньої»** викликає RPC **`organizer_refresh_match_stage_link_latest`** (потрібні **`share_group_id`** у рядку та нова view-публікація в тій самій групі).

...
- У **`PortalShell`**: «Увійти» або в один ряд — бейджі **Учасник** / **Організатор** (за тими ж правилами, що нижче), **іконка облікового запису** (email лише в підказці та `aria-label`), кнопка виходу. Статус **`pending`** у шапку **не виводиться**.

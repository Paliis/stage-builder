...
| **`match_registrations`** | Одна заявка на пару (**match**, **стрілок**): `division`, клас, PF, `categories` JSONB, `status` pending/confirmed/cancelled (організатор може ставити **`cancelled`**), `payment_note`, **`phone`**, **`weapon_details`**, **`competitor_region`**, `confirmed_at/by`. **Відіза стрільця** з картки матчу: **`DELETE`** рядка **`pending`** або **`cancelled`** (до підтвердження), потім знов **`INSERT`** — див. **`20260525100000_...`**. Старий **`cancelled`** рядок перед повторною підачею клієнт також **`DELETE`**-ить, потім **`INSERT`**. |

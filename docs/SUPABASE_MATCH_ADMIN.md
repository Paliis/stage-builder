...
- **`match_registrations`:** стрілець читає/створює свою заявку на **published** матч; **скасувати непідтверджену заявку** — **`DELETE`** власного рядка (**`pending`/`cancelled`**, **`confirmed_*` IS NULL**), **`20260525100000_...`**; повторна реєстрація — звичайний **`INSERT`**. Опційно **`20260524120000_...`** лишає **`UPDATE`** `cancelled`→`pending` для сумісності. Організатор читає та **оновлює** лише коли активний; організатор — підтвердження офлайн оплати (`payment_note`, `status`, `confirmed_*`). **Anonymous** таблицю не бачить (списки без логіну — окремий RPC пізніше).

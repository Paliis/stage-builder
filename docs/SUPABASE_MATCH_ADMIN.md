...
11. **`supabase/migrations/20260510120000_participant_registration_defaults.sql`** (+ **`20260504180000_participant_defaults_region_category_weapon.sql`**) — таблиця **`participant_registration_defaults`** (типові **`division`**, **`classification_grade`**, **`power_factor`**, а також **`region`**, **`category`**, **`weapon_class`** для PractiScore-style експорту та префілу; **`SELECT` / `INSERT` / `UPDATE` / `DELETE`** лише власного рядка **`user_id`**).
...
| **`participant_registration_defaults`** | Один рядок на **стрільця**: типові поля (`division`, `classification_grade`, **`power_factor`**, **`region`**, **`category`**, **`weapon_class`**) для автопідстановки та експорту; якщо заявки на цей матч ще немає — префіл у публічну форму (**`/{locale}/account`** або картка матчу). |

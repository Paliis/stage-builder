...
- У **`PortalShell`**: явний стан («Увійти» або email), бейдж **учасника** для всіх залогінених користувачів; маркер **організатора** лише якщо в **`match_admin_profiles.organizer_status`** значення **`active`**; при **`blocked`** — бейдж **обмежено**. Статус **`pending`** у шапку **не виводиться** — інакше плутається з майбутнім кабінетом стрільця (`pending` використовує й технічний рядок профілю перед схваленням платформи).

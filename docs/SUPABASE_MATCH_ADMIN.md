...
| **`match_registrations`** | Одна заявка на пару (**match**, **стрілок**): `division`, клас, PF, `categories` JSONB, `status` pending/confirmed/cancelled, `payment_note`, **`phone`**, **`weapon_details`**, **`competitor_region`** (знімок регіону з профілю на момент заявки), `confirmed_at/by`. |

...
**Останнє оновлення документа:** травень 2026 (окремі розділи **§2.3–2.4** про портал і матчі; узгоджено з [PRODUCT.md](./PRODUCT.md) **A.5–A.6**).
...
| **Модуль «Матчі» (опційно на проді)** | Каталог, реєстрація, кабінет організатора, **`.psc`** — увімкнення збіркою **`VITE_ENABLE_MATCH_PORTAL`**. Бізнес-опис — **§2.4**; техніка — **§3.2–3.3** |
...
### 2.3. Портал Shooters Tools (загально)

Один домен (**[shooters-tools.com](https://shooters-tools.com)**), **одне** React-застосування: локалізовані сторінки під **`/:locale`** (`uk`/`en`), спільний **PortalShell**, довідникові блоки (**RO Helper**, **Hit Factor**, політика публікації), вхід у **акаунт** (Supabase Email). Редактор **Stage Builder** відкривається як **`/stage-builder`** і залишається основним «важким» модулем продукту; навколо нього — сервіси для стрільців і організаторів. Продуктова карта порталу (модулі, freemium, UX) — [PORTAL_PLAN.md](./PORTAL_PLAN.md); коди розділів у планах — [PLANNING_INDEX.md](./PLANNING_INDEX.md) (**PT**).

### 2.4. Модуль «Матчі»

**Бізнес-сенс:** оголошення змагань, набір учасників (скводи, ліміти), кабінет організатора, зв’язок вправ із **Stage Builder** (share), вивантаження в **PractiScore** (**.psc**). Організатори проходять **модерацію** власником платформи; є **сповіщення про заявки організаторів** (див. [SUPABASE_ORGANIZER_APPLICATION_ALERTS.md](./SUPABASE_ORGANIZER_APPLICATION_ALERTS.md)).

**На проді** розділ **не показують** без **`VITE_ENABLE_MATCH_PORTAL=1`** на збірці (див. [MATCH_REGISTRATION_AND_PSC_PLAN.md §1.4](./MATCH_REGISTRATION_AND_PSC_PLAN.md)). План робіт і статус реалізації — [MATCH_PORTAL_PRODUCT_PLAN.md](./MATCH_PORTAL_PRODUCT_PLAN.md), беклог **MA-*** — [BACKLOG_MATCHES.md](./BACKLOG_MATCHES.md).

### 2.5. Обмеження (бізнес-рівень)
...
- **Акаунт порталу** (Supabase) потрібен для **реєстрації на матчі** та налаштувань стрільця (**§2.4**); він **не** є хмарним сховищем файлів **Stage Builder**.
...
| **PRODUCT.md** | Бізнес + технічний огляд; **портал і матчі** — **A.5**, **A.6** |

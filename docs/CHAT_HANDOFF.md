...
У режимі `portal-shell--app` навігація по продуктах завжди в бургер-каталозі, а акаунт і перемикач мови лишаються в смузі; редактор віддав шеллу бренд-лінк, перемикач мови і футер. Висоту смуги редактор враховує через `--app-chrome-offset` (задає `.portal-shell--app`, споживає `--_dvh-cap` у `src/App.css`).

...
**Відновлення пароля** (`PortalCompactEmailAuth`, три кроки): «Забули пароль?» → адреса → `resetPasswordForEmail` → **код із листа** (`verifyOtp` з `type: 'recovery'`) → `updateUser({ password })` у тій самій формі. Посилання з листа свідомо **не** використовуємо: воно приходить у спам, фільтри відкривають його першими й одноразовий токен згорає — на проді це давало `403 otp_expired`. Умова з Dashboard: у шаблоні **Reset password** має бути `{{ .Token }}`. Окремо є `src/portal/account/AccountPasswordSection.tsx` — зміна пароля для того, хто вже ввійшов.
...
## «Зона стрільця» в 3D (BL-045)

Камера більше не стоїть у захардкоджених координатах — точки огляду рахує `src/domain/shooterViewpoint.ts`:

1. маркери **стартової позиції** (кілька — перемикач `‹ Старт n/N ›` біля вкладок камери; порядок знизу вгору по плану);
2. без них — центр габаритів **штрафних ліній і зовнішніх контурів штрафних зон**;
3. без розмітки — середина ширини поля на чверті довжини.

Око на 1,6 м, точка притискається всередину поля (на 15×20 камера раніше опинялась за задньою стінкою). Маркер старту симетричний, тому його поворот задає лише вісь погляду — бік обирається в бік центра мас мішеней. Панорама більше не опускає камеру під підлогу (`MIN_CAMERA_EYE_Y_M` у `StageNavigator`).

Відомі обмеження: «всередині штрафних» гарантується лише центром габаритного прямокутника — для Г-подібного коридору центр може випасти поза розміткою.
...
```
Прочитай docs/CHAT_HANDOFF.md. Далі: за потреби MA-C03/PSC або UX/доки. commit + push main після змін.
```

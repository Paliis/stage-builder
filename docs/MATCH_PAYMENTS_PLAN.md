...
**Рівень організатора (один раз):** підключення **Monobank Acquiring** — поле **X-Token** (маскується після збереження), кнопка **«Перевірити»** — лише перевірка токена (**`GET /api/merchant/pubkey`**, без тестового інвойсу / без грошей; тест 1 UAH — опційно пізніше).  

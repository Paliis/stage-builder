...
У Dashboard: **Authentication** → **Email Templates** — шаблони **Confirm signup** тощо; у HTML використовуйте змінні з підказок редактора (`{{ .ConfirmationURL }}`, `{{ .SiteURL }}`). Якщо в шаблоні є **`{{ .Token }}`** (часто **6 або 8** цифр залежно від налаштувань проєкту), клієнт порталу після реєстрації пропонує ввести код і викликає **`verifyOtp`** — лінк у листі для підтвердження не обов’язковий (можна прибрати `{{ .ConfirmationURL }}`, щоб не провокувати сканери пошти). Тема листа — у тому ж екрані.
...
Форма входу має «Забули пароль?» → `resetPasswordForEmail` → **код із листа** (`verifyOtp` з `type: 'recovery'`) → новий пароль просто у формі. Тому шаблон **Reset password** у Dashboard **має містити `{{ .Token }}`**; без нього код у листі не з'явиться.

Чому код, а не посилання: листи часто йдуть у спам, а фільтри й сканери відкривають `{{ .ConfirmationURL }}` раніше за адресата. Токен одноразовий, тож людина отримує **`403 otp_expired`** («Email link is invalid or has expired») — саме це ми й ловили на проді. З тієї ж причини `{{ .ConfirmationURL }}` варто прибрати з шаблонів **Confirm signup** і **Reset password**: коду достатньо.

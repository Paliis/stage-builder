...
- **Поділитися посиланням** — після згоди з політикою публікації можна отримати URL перегляду або редактора (хмара Supabase; див. `docs/BL-001_SHARE_LINK_PLAN.md`).
...
**Публікація посилань (share):** у змінних Vercel задайте **`SUPABASE_SERVICE_ROLE_KEY`**, **`SUPABASE_URL`** або **`VITE_SUPABASE_URL`**; опційно **`VITE_SHARE_PUBLIC_ORIGIN`** для абсолютних URL у відповіді API. Звичайний **`npm run dev`** не виконує serverless-функції — для перевірки **`POST /api/publish-share`** використовуйте **`vercel dev`** або preview/production deploy.

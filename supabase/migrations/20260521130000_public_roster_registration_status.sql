-- Public participant list: Confirmation column aligns with organizer roster (registration status).
...
COMMENT ON FUNCTION public.fetch_public_match_roster(UUID) IS
  'Published matches, open list: registrations with status pending|confirmed — same status as organizer Applications table; shooter categories[]. No user ids / no payment memo.';

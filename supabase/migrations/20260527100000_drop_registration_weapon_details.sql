-- Remove unused free-text weapon_details from registrations and shooter defaults;
-- organizer roster RPC no longer exposes this column.
...
COMMENT ON FUNCTION public.fetch_organizer_match_registration_roster(UUID) IS
  'Match owner: roster with squad, payment fields, phone, region, participant_payment_option; display_name from participant last+first name then match_admin_profiles.';

ALTER TABLE public.match_registrations DROP COLUMN IF EXISTS weapon_details;
ALTER TABLE public.participant_registration_defaults DROP COLUMN IF EXISTS weapon_details;

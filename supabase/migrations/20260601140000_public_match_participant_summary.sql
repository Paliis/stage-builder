-- MA-E03: public participant counts by division and category (published + open roster).
...
COMMENT ON FUNCTION public.fetch_public_match_participant_summary(UUID) IS
  'Published match, open roster: registration counts by division and category (pending + confirmed). Multi-category shooters counted in each category row.';

GRANT EXECUTE ON FUNCTION public.fetch_public_match_participant_summary(UUID) TO anon, authenticated;

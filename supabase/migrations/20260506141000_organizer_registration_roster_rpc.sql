-- Organizer roster rows with display_name (profiles are not readable cross-user under RLS).
...
COMMENT ON FUNCTION public.fetch_organizer_match_registration_roster(UUID) IS
  'Match owner: registration list with optional display_name for organizer UI.';

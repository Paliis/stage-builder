-- Organizer roster: display_name from shooter defaults (last + first), then match_admin_profiles.
...
COMMENT ON FUNCTION public.fetch_organizer_match_registration_roster(UUID) IS
  'Match owner: roster with squad, payment fields, phone, weapon, region, participant_payment_option; display_name from participant last+first name then match_admin_profiles.';

...
export const ukMessages: MessageTree = {
  ...
  view: {
    ...
    threeDControlsDetail:
      'Режим «Як у PDF» показує той самий кадр, що потрапить у документ (рамка — межі кадру). Знімок для PDF має ті ж пропорції. У PDF: QR у верхньому правому куті знімка; посилання та «згенеровано…» — під знімком.',
    plan2dControls:
...
export const enMessages: MessageTree = {
  ...
  view: {
    ...
    threeDControlsDetail:
      'Use «PDF frame» to preview the same crop as the briefing snapshot (dashed outline). The exported PNG uses the same proportions. In the PDF the QR sits in the snapshot top-right; the brand line and URL are below the image.',
    plan2dControls:

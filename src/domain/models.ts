...
export type PropType =
  ...
  | 'shieldWithPortDoor'
  | 'barrel'
  /** Дві бочки одна на одній: висота 3D **рівно 2×** від однієї колони `barrel` (`BARREL_DOUBLE_HEIGHT_M` = 2·`BARREL_COLUMN_HEIGHT_M`). */
  | 'barrelDouble'
  | 'tireStack'
  /** Стос шин ~1 м (на плані той самий діаметр, що й `tireStack`). */
  | 'tireStack1m'
  /** Стос шин ~1,5 м (на плані той самий діаметр, що й `tireStack`). */
  | 'tireStackTall'
  ...
  | 'weaponRackPyramid'
  /** Декор «авто»: слід на плані як прямокутник; у 3D — спрощений блок (без коліс). Орієнтація: довга вісь `sizeM.x` — поздовжня вісь кузова. */
  | 'decorationCar'
